...
for command in awk cmp docker flock git grep jq mktemp sed sha256sum stat uv; do
...
uv run python scripts/qa/source_manifest.py \
    --root "$ROOT" --output "$EVIDENCE_DIR/source-manifest.json"
uv lock --check
...
[ "$FILESYSTEM_MISCONFIGURATIONS" -eq 0 ] || fail filesystem_scan_failed
...
jq -n \
    ...
    '{schema_version:1,run_id:$run_id,status:$status,image_digest:$image_digest,gates:{source_credential_patterns:"PASS",filesystem_gitleaks:"PASS",git_history_gitleaks:"PASS",python_dependency_audit:"PASS",action_sha_pins:"PASS",image_history_and_config_gitleaks:"PASS",pinned_base_public_gpg_key_verified:$base_public_gpg_key_verified,image_trivy:"PASS",filesystem_trivy:"PASS"},scanner_images:{gitleaks:$gitleaks_image,trivy:$trivy_image,python_base:$python_base_image},counts:{history_commits:$history_commits,image_high_critical_vulnerabilities:$image_vulnerabilities,image_secrets:$image_secrets,filesystem_high_critical_misconfigurations:$filesystem_misconfigurations}}' \
