...
[ "$running" = true ] || fail
run_label=$(docker inspect \
    --format '{{index .Config.Labels "nvidia-build-lb.run"}}' \
    "$DB_CONTAINER" 2>/dev/null) || fail
[ -n "$run_label" ] && [ "$run_label" != '<no value>' ] || fail
...
jq -n \
    ...
    '{schema_version:1,alembic_revision:$revision,upstream_count:$upstream_count,upstream_identity_sha256:$upstream_identity_sha256,upstream_keys:$upstream_keys,downstream_count:$downstream_count,downstream_digest_sha256:$downstream_digest_sha256}' \
    || fail
