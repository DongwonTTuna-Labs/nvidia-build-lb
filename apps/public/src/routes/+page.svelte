...
function stateLabel() {
  if (viewState === "ready") return "트래픽 준비 완료";
  if (viewState === "degraded") return "운영 준비 중";
  if (viewState === "offline") return "연결 확인 필요";
  return "상태 확인 중";
}

function stateMessage() {
  if (viewState === "ready") return "요청을 안전하게 전달할 수 있습니다.";
  if (viewState === "degraded") {
    return "게이트웨이는 응답하지만 아직 활성 upstream이 없어 트래픽을 받지 않습니다.";
  }
  if (viewState === "offline") return "게이트웨이에 연결할 수 없습니다. 잠시 후 다시 시도하세요.";
  return "게이트웨이의 현재 상태를 확인하고 있습니다.";
}
...
<main class="shell">
  ...
  <section class="hero" aria-labelledby="page-title">
    ...
    <div class:ready={viewState === "ready"} class:offline={viewState === "offline"} class="status-card">
      ...
      <div class="status-meta">
        ...
        <button type="button" class="refresh" on:click={refresh} disabled={refreshing}>
  ...
  <section class="metrics" aria-label="게이트웨이 핵심 상태">
    <article class="metric-card accent">
      ...
      <p>{health.eligible_keys === 2 ? "분산 라우팅 가능" : "관리자 설정 대기"}</p>
    </article>
  ...
  <section class="content-grid" aria-label="지원 API와 운영 안내">
    <article class="panel api-panel">
      <div class="panel-heading">
        ...
        <span class="panel-count">06</span>
      </div>
      <div class="api-list">
        ...
        <div class="api-row"><span class="api-icon">▣</span><span><strong>Image &amp; Video</strong><small>생성·변환</small></span><code>POST /v1/images/*</code></div>
        <div class="api-row"><span class="api-icon">◉</span><span><strong>Audio</strong><small>speech · transcription</small></span><code>POST /v1/audio/*</code></div>
...
<style>
  ...
  .status-card p { min-height: 48px; color: #afbbb4; font-size: 13px; line-height: 1.6; }
  .status-meta { justify-content: space-between; gap: 12px; border-top: 1px solid #33432f; padding-top: 16px; color: #78877e; font-size: 11px; }
  .refresh { border: 0; background: transparent; color: #9dd54a; cursor: pointer; font-size: 11px; font-weight: 700; }
  ...
  .metric-card strong small { color: #637160; font-size: .55em; }
  .metric-card p { margin: 8px 0 0; color: #7e8b85; font-size: 12px; }
  ...
  .panel-count { color: #4f5e58; font-size: 28px; font-weight: 800; }
  ...
  .api-row small { margin-top: 5px; color: #75837c; font-size: 11px; }
  code { color: #66806b; font-size: 10px; white-space: nowrap; }
