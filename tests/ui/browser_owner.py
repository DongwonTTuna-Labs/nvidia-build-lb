...
from .browser_checks import assert_no_page_overflow, clipboard_is_empty
...
def _exercise_busy_probe(session: AuthenticatedSession, state: FakeAdminState) -> str:
    ...
    expect(busy_row.locator("td[data-label='Health']")).to_have_text("healthy · success")
...
def run_upstream_journey(
    ...
    expect(created_row.locator("td[data-label='Health']")).to_have_text("healthy · success")
    ...
    expected_focus = "key-00000000-0000-4000-8000-000000000002-toggle"
    assert page.locator(":focus").get_attribute("id") == expected_focus
    assert_no_page_overflow(page)
    recorder.capture(
        page,
        CaptureSpec(
            name=f"{capture_prefix}admin-upstream-post-cleanup",
            state="probe enable disable delete completed",
            viewport="1280x900",
